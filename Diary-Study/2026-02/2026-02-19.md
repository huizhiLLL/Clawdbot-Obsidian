...
### 问题 1
在 SQL 中，要查询年龄大于等于 18 且小于 30 的用户，应该使用哪个 WHERE 条件？

A. `WHERE age > 18 AND < 30`
B. `WHERE age >= 18 AND age < 30`
C. `WHERE age BETWEEN 18 AND 30`
D. `WHERE age > 18 OR age < 30`

### 问题 2
哪个运算符用于检查值是否在一组给定的值中？
...
### 问题 3
在 LIKE 运算符中，`%` 和 `_` 分别表示什么？

A. `%` 表示任意数量的字符，`_` 表示单个字符
B. `%` 表示单个字符，`_` 表示任意数量的字符
C. `%` 表示空格，`_` 表示下划线
D. `%` 表示通配符，`_` 表示转义字符
