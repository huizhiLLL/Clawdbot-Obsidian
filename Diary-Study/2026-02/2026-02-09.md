...
**答案：**
- `{{ }}`（文本插值）：将数据渲染为纯文本，HTML 标签会被转义，不会解析
- `v-html`：将数据渲染为 HTML，标签会被浏览器解析

**示例：**
```vue
<script setup>
const content = "<strong>粗体文字</strong>"
</script>

<!-- 输出：<strong>粗体文字</strong>（纯文本） -->
<p>{{ content }}</p>
...
### 问题 2：为什么 `v-for` 必须使用 `:key`？

**答案：**
`:key` 帮助 Vue 识别每个节点的唯一性，提高渲染性能。没有 `:key` 时，Vue 会使用就地复用策略，可能导致：
- 状态错乱（比如输入框内容跑到了错误的位置）
- 过渡动画失效
- 性能下降

**最佳实践：**
- 使用唯一的、稳定的值作为 key（如 id）
...
<!-- ❌ 差：使用 index（列表有增删时会有问题） -->
<li v-for="(item, index) in items" :key="index">{{ item.name }}</li>
```

---

### 问题 3：`v-if` 和 `v-show` 有什么区别？什么时候用哪个？
...
| ---- | ---------- | ---------- |
| 渲染   | 条件为真时渲染 | 始终渲染       |
| 销毁   | 条件为假时销毁 | 只是切换 display: none |
| 初始开销 | 小          | 大（因为要渲染）    |
| 切换开销 | 大          | 小（只是改变样式）   |

**使用建议：**
- 用 `v-if`：条件很少改变、初始化时不确定是否需要显示
- 用 `v-show`：需要频繁切换显示/隐藏、初始化时肯定要渲染

```vue
<!-- 场景 1：登录状态（很少改变，用 v-if）-->
<div v-if="isLoggedIn">欢迎回来</div>

<!-- 场景 2：标签页切换（频繁切换，用 v-show）-->
...
**注意：** `index` 从 0 开始，所以显示时要 `+ 1`

---

## 💡 学习小贴士

1. **插值语法**：只用于显示文本，不要在 `{{ }}` 里写复杂逻辑，用计算属性代替
2. **指令简写**：养成使用 `:` 和 `@` 简写的习惯，代码更简洁
3. **v-for 的 key**：永远不要忘记 `:key`，这是 Vue 性能优化的关键
4. **事件修饰符**：`.prevent`、`.stop`、`.once` 等修饰符很有用，记住常用几个
