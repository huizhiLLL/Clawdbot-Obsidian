# 2026-02-14 学习记录

**主题：** Node.js 模块系统

**分类：** Node.js 基础

**难度：** beginner

---

## 📌 核心知识点

### 1. CommonJS 规范

Node.js 默认使用 CommonJS 模块系统，主要通过 `require()` 和 `module.exports` 来导入和导出模块。

#### 导入模块
```javascript
// 导入内置模块
const fs = require('fs');
const path = require('path');

// 导入第三方模块
const express = require('express');

// 导入本地模块（相对路径）
const utils = require('./utils');
const myModule = require('./myModule/index');
```

#### 导出模块
```javascript
// 方式 1：导出单个值
module.exports = function() {
  console.log('Hello!');
};

// 方式 2：导出对象
module.exports = {
  name: 'MyModule',
  sayHello() {
    console.log('Hello!');
  }
};

// 方式 3：使用 exports（注意：不能直接赋值 exports）
exports.name = 'MyModule';
exports.sayHello = function() {
  console.log('Hello!');
};
```

### 2. ES Module (ESM)

Node.js 也支持 ES6 模块语法，通过 `.mjs` 扩展名或在 `package.json` 中设置 `"type": "module"` 来启用。

#### 导入模块
```javascript
// 导入内置模块（需添加 node: 前缀）
import fs from 'node:fs';
import path from 'node:path';

// 导入第三方模块
import express from 'express';

// 导入本地模块
import utils from './utils.js';
import { sayHello } from './myModule.js';
```

#### 导出模块
```javascript
// 默认导出
export default function() {
  console.log('Hello!');
}

// 命名导出
export const name = 'MyModule';
export function sayHello() {
  console.log('Hello!');
}

// 混合导出
const util1 = () => {};
const util2 = () => {};
export { util1, util2 };
export default { util1, util2 };
```

### 3. 模块加载机制

```javascript
// Node.js 模块查找顺序：
// 1. 内置模块（如 fs、path）
// 2. 相对路径模块（./ 或 ../ 开头）
// 3. 绝对路径模块（/ 开头）
// 4. node_modules 中的模块

// require() 会依次查找：
// 1. .js 文件
// 2. .json 文件（会自动 JSON.parse）
// 3. .node 文件（C++ 插件）
// 4. 目录中的 index.js
// 5. package.json 中 main 字段指定的文件
```

### 4. 模块缓存

```javascript
// 模块在首次 require 后会被缓存
// 多次 require 同一模块，只会执行一次
const mod1 = require('./myModule');
const mod2 = require('./myModule');
console.log(mod1 === mod2); // true

// 清除模块缓存
delete require.cache[require.resolve('./myModule')];
```

### 5. __dirname 和 __filename

```javascript
// CommonJS 中可用
console.log(__dirname); // 当前文件所在目录的绝对路径
console.log(__filename); // 当前文件的绝对路径

// ESM 中需要使用 url 模块
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
```

### 6. package.json 中的模块配置

```json
{
  "name": "my-package",
  "version": "1.0.0",
  "type": "module",        // 使用 ESM
  "main": "index.js",      // CommonJS 入口
  "exports": {             // 现代模块导出配置
    ".": "./index.js",
    "./utils": "./utils.js"
  },
  "dependencies": {
    "express": "^4.18.0"
  }
}
```

## 🎯 学习进度
- [ ] 概念理解
- [ ] Demo 完成
- [ ] 延伸阅读

## 💡 收获与感悟
（学习后填写）

## 🤔 疑问与待解决
（学习后填写）

## 📚 延伸阅读
- [Node.js Modules 官方文档](https://nodejs.org/api/modules.html)
- [ECMAScript Modules in Node.js](https://nodejs.org/api/esm.html)
- [CommonJS vs ES Modules](https://nodejs.org/api/esm.html#differences-between-es-modules-and-commonjs)
